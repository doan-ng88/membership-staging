...
<template>
    <DefaultLayout>
      <div class="p-6">
        <PageHeader>
          <template #title>
            <h2 class="text-2xl font-bold text-gray-800">{{ t('callCampaign.createCallCampaign.title') }}</h2>
          </template>
        </PageHeader>
  
        <a-form ref="formRef" :model="formState" :rules="rules" layout="vertical">
          <div class="flex gap-4">
            <a-form-item :label="t('callCampaign.createCallCampaign.form.name.label')" name="name" class="flex-1">
              <a-input v-model:value="formState.name" placeholder="Enter campaign name" />
            </a-form-item>
            <a-form-item
              :label="t('callCampaign.createCallCampaign.form.description.label')"
              name="description"
              class="flex-1"
            >
              <a-input
          ...
          <div class="flex gap-4">
            <a-form-item
              :label="t('callCampaign.createCallCampaign.form.website.label')"
              name="websiteId"
              class="flex-1"
            >
              <a-select
            ...
            <a-form-item
              :label="
                t(
                  formState.websiteId
                    ? 'callCampaign.createCallCampaign.form.coupon.label'
                    : 'callCampaign.createCallCampaign.form.coupon.labelWithWebsite'
                )
              "
              name="coupons"
              class="flex-1"
          ...
          <div class="flex gap-4">
            <a-form-item :label="t('callCampaign.createCallCampaign.form.pic.label')" name="pic" class="flex-1">
              <a-select
            ...
            <a-form-item
              :label="t('callCampaign.createCallCampaign.form.time.label')"
              name="startDate"
              class="flex-1"
            >
              <a-range-picker
                ...
                format="YYYY-MM-DD"
                :placeholder="[
                  t('callCampaign.createCallCampaign.form.time.startPlaceholder'),
                  t('callCampaign.createCallCampaign.form.time.endPlaceholder')
                ]"
                style="width: 100%"
          ...
          <div class="mt-6 pt-4 border-t border-gray-200">
            <div class="flex justify-between items-center mb-4">
              <h4 class="text-lg font-medium">{{ t('callCampaign.createCallCampaign.memberList.title') }}</h4>
              <a-button type="primary" @click="showAddMemberModal">
                <template #icon><UserAddOutlined /></template>
                {{ t('callCampaign.createCallCampaign.memberList.addButton') }}
              </a-button>
            </div>
          ...
          <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4">
            <div class="max-w-7xl mx-auto flex justify-end gap-4">
              <a-button @click="handleBack">{{ t('callCampaign.createCallCampaign.buttons.back') }}</a-button>
              <a-button type="primary" @click="handleSubmit">{{
                t('callCampaign.createCallCampaign.buttons.submit')
              }}</a-button>
            </div>
          </div>
        </a-form>
  ...
  import type { Campaign, CampaignMember } from '../../mail/types/mail.types'
  ...
  const memberColumns = computed(() => [
    {
      title: t('callCampaign.createCallCampaign.memberList.table.customerName'),
      dataIndex: 'fullName',
      key: 'fullName',
      width: '20%'
    },
    {
      title: t('callCampaign.createCallCampaign.memberList.table.phoneNumber'),
      dataIndex: 'mainPhoneNumber',
      key: 'mainPhoneNumber',
      width: '20%'
    },
    {
      title: t('callCampaign.createCallCampaign.memberList.table.website'),
      dataIndex: 'websiteName',
      key: 'websiteName',
      width: '20%'
    },
    {
      title: t('callCampaign.createCallCampaign.memberList.table.birthday'),
      dataIndex: 'birthday',
      key: 'birthday',
      width: '20%'
    },
    {
      title: t('callCampaign.createCallCampaign.memberList.table.registeredTime'),
      dataIndex: 'registeredTime',
      key: 'registeredTime',
      width: '20%'
    },
    {
      title: t('callCampaign.createCallCampaign.memberList.table.actions'),
      key: 'action',
      width: '10%'
    }
  ])
  ...
  const rules = computed(() => ({
    name: [{ required: true, message: t('callCampaign.createCallCampaign.form.name.required') }],
    startDate: [{ required: true, message: t('callCampaign.createCallCampaign.form.time.required') }],
    pic: [{ required: true, message: t('callCampaign.createCallCampaign.form.pic.required') }],
    websiteId: [{ required: true, message: t('callCampaign.createCallCampaign.form.website.required') }]
  }))
  ...
  const fetchAdminUsers = async () => {
    try {
      ...
      message.error(t('callCampaign.createCallCampaign.messages.error.fetchAdmins'))
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formState.members.length === 0) {
        message.error(t('callCampaign.createCallCampaign.messages.selectMember'))
        return
      }
    } catch (error) {
    ...
    try {
      ...
      message.success(t('callCampaign.createCallCampaign.messages.success'))
      router.back()
    } catch (error) {
      message.error(t('callCampaign.createCallCampaign.messages.error.default'))
    }
  }
  ...
  const fetchCoupons = async (search?: string) => {
    ...
    try {
      ...
      message.error(t('callCampaign.createCallCampaign.messages.error.fetchCoupons'))
  ...
  const tableConfig = computed(() => ({
    pagination: {
      ...
      showTotal: (total: number) =>
        t('callCampaign.createCallCampaign.memberList.total', { total: formState.members.length }),
